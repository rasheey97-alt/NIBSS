...
public class TransactionService {
    ...
    public void calculateAndSetCommission() {
        ...
        // Loop through each successful transaction
        for (Transaction transaction : successfulTransactions) {
            double transactionAmount = transaction.getAmount;

            // Calculate the transaction fee with a cap of $100 (0.5% of the amount)
            double transactionFee = Math.min(transactionAmount * 0.005, 100);

            // Calculate the commission, which is 20% of the transaction fee
            double commission = transactionFee * 0.2;

            // Update the transaction's properties with the calculated values
